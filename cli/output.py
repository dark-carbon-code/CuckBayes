...
from scripts.prank_payloads import run_all_pranks
...
def show_results(result: dict):
    ...
    console.print(f"🧠 Probability: {probability:.0%}", style="bold cyan")
    console.print(f"📉 Meter: {meter}\n", style="bold magenta")

    # Flavor Text
    if label == "Cuckman":
        console.print("🔻 Consider removing the cuck chair. And the Signal app.")
        run_all_pranks()  # 🔥 Trigger prank payload
    else:
    ...
    message = f"""[CuckBayes Broadcast Alert: {user_name}]
Archetype: {title}
